...
fh = logger.FileHandler(f"./log/{time_str}lstm.txt")
...
def train(model, device, train_loader, valid_loader, criterion, optimizer):
    ...
    for epoch in tqdm(range(1, EPOCHS+1)):
        ...
        if np.mean(valid_acc) >= valid_acc_max:
            ...
            torch.save(model.state_dict(), f"./checkpoints/{time_str}model.pt")
        ...
        recorder.update(epoch, np.mean(train_loss), np.mean(train_acc), np.mean(valid_loss), np.mean(valid_acc))
...
class RecorderMeter:
    ...
    def plot_curve(self, save_path):
        ...
        plt.plot(x_axis, y_axis, color="g", linestyle=":", label="train-loss-x300", lw=2)
        plt.legend(loc=4, fontsize=legend_fontsize)

        y_axis[:] = self.epoch_losses[:, 1]
        plt.plot(x_axis, y_axis, color="y", linestyle=":", label="valid-loss-x300", lw=2)
        plt.legend(loc=4, fontsize=legend_fontsize)
